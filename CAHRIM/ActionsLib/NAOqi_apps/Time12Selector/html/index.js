...
function enterTime(time) {
    session.service("ALMemory").done(function (ALMemory) {
        ALMemory.raiseEvent("CARESSES/DateTimeSelector/time12", time);
    });    
}
...
function addNumber(element){
    ...
    if (element.value == "  x  "){
        ...
        if (complete == false){
            if ((element.value != ' am ') && (element.value != ' pm ')){
        ...
        if ((element.value == ' am ') || (element.value == ' pm ')){
            ...
            if (str.charAt(6) == 'a'){
                document.getElementById('mvar').value = str.replace("am", val);
            } else if (str.charAt(6) == 'p'){
                document.getElementById('mvar').value = str.replace("pm", val);
            }
        }
    }
